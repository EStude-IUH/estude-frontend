...
const navItems = [
  ...
  { icon: BookOpenCheck, label: 'Lớp học' },
  { icon: UsersRound, label: 'Học viên' },
  { icon: ClipboardCheck, label: 'Bài tập & chấm điểm' },
  { icon: CalendarClock, label: 'Lịch giảng dạy' },
...
export function StaffDashboardView() {
  ...
  return (
    <div className="min-h-screen bg-[#f5f7fb] text-slate-950">
      <header className="fixed inset-x-0 top-0 z-40 flex h-[74px] items-center border-b border-slate-200 bg-white/95 px-4 backdrop-blur lg:pl-[286px] lg:pr-8">
        <button
          type="button"
        ...
        <div className="relative hidden w-full max-w-md sm:block">
          ...
          <input
            type="search"
            aria-label="Tìm kiếm"
            placeholder="Tìm lớp học, học viên, bài tập..."
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-[266px] flex-col border-r border-slate-800 bg-slate-950 px-4 py-6 text-white transition-transform duration-300 lg:translate-x-0 ${
      ...
      >
          ...
          {navItems.map(({ icon: Icon, label, active }) => (
            <button
              ...
              className={`flex w-full items-center gap-3 rounded-xl px-3.5 py-3 text-sm font-semibold transition ${
                active
                  ? 'bg-gradient-to-r from-indigo-600 to-blue-600 text-white shadow-lg shadow-indigo-950/30'
        ...
        <div className="mt-auto space-y-1.5 border-t border-white/10 pt-5">
          <button
            type="button"
            className="flex w-full items-center gap-3 rounded-xl px-3.5 py-3 text-sm font-semibold text-slate-400 hover:bg-white/[0.07] hover:text-white"
          >
          ...
          <button
            ...
            className="flex w-full items-center gap-3 rounded-xl px-3.5 py-3 text-sm font-semibold text-rose-300 transition hover:bg-rose-500/10 disabled:opacity-60"
          >
            {isSigningOut ? (
      ...
      <main className="min-h-screen px-4 pb-10 pt-[98px] sm:px-6 lg:ml-[266px] lg:px-8">
            ...
            <div className="relative flex flex-col justify-between gap-7 lg:flex-row lg:items-end">
              <div>
                <span className="inline-flex items-center gap-2 rounded-full border border-cyan-300/20 bg-cyan-300/10 px-3 py-1.5 text-xs font-bold text-cyan-200">
                  <Sparkles className="size-3.5" /> Trung tâm điều hành EStude
                </span>
                ...
                <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300 sm:text-base">
                  Theo dõi lớp học, tiến độ học viên và các công việc cần xử lý
