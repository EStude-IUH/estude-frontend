...
async function renderComponent(t, file, exportName, api, options = {}) {
  ...
  loaded.require = (id) => {
    if (id === "next/navigation") return { useRouter: () => ({ push() {} }), useParams: () => ({ materialId: options.materialId ?? "doc" }) };
...
test("mastery explains evidence, shows filter counts and filters locally", async (t) => {
  ...
  const ui = await renderComponent(t, "components/student/study-coach-mastery-page.tsx", "StudyCoachMasteryPage", { getMastery: async (query) => { calls.push(query); return { items: calls.length === 1 ? [item] : [], total: calls.length === 1 ? 1 : 0 }; }, getMaterial: async () => ({ id: "doc", title: "cells.pdf" }) }, { search: "?documentId=doc" });
  assert.match(ui.text(), /Tế bào/); assert.match(ui.text(), /Cần ôn lại/); assert.match(ui.text(), /Dễ/); assert.match(ui.text(), /Study Coach đánh giá như thế nào/);
  await act(async () => { ui.button("Nắm rất vững").props.onClick(); await flush(); });
  assert.deepEqual(calls, [{ documentId: "doc" }]); assert.match(ui.text(), /Chưa có khái niệm ở mức/); assert.match(ui.text(), /Tiếp tục làm Quiz/);
});
...
test("insights render safe ready/fallback states, preserve action order and prevent duplicate generation", async (t) => {
  ...
  assert.match(ui.text(), /gợi ý tạm thời/i); assert.match(ui.text(), /Luyện quiz/); assert.match(ui.text(), /Tế bào/);
});
...
test("insights with no evidence show objective guidance instead of strengths or weaknesses", async (t) => {
  ...
  assert.match(ui.text(), /Chưa đủ dữ liệu để đánh giá khách quan/);
